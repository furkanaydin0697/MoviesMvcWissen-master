    ...
    public class MoviesController : Controller
    {
        ...
        private string CreateFilePath(string Name, HttpPostedFileBase Image)
        {
            ...
            if (Image != null && Image.ContentLength > 0)
            {
                ...
                DateTime.Now.Second.ToString().PadLeft(2, '0') + DateTime.Now.Millisecond.ToString().PadLeft(3, '0') + "_" +
                Name.Replace(" ", "") + Path.GetExtension(Image.FileName);
                if (Path.GetExtension(fileName).ToLower().Equals(".jpg") || Path.GetExtension(fileName).ToLower().Equals(".png") ||
                    Path.GetExtension(fileName).ToLower().Equals(".jpeg") || Path.GetExtension(fileName).ToLower().Equals(".bmp"))
                {
